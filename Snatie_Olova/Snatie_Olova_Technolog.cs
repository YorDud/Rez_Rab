...
{
	public partial class Snatie_Olova_Technolog : Form
	{
		public Snatie_Olova_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Sn_Olov_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Snatie_Olova_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Sn_Olov_Correction" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				//
				var Sn_Olov_Correction = dataGridView1.Rows[e.RowIndex].Cells["Sn_Olov_Correction"].Value;
				//
        ...
        UPDATE Snatie_Olova SET
            [Date_tech] = @Date_tech,
            [Sn_Olov_Correction] = @Sn_Olov_Correction
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					//
					command.Parameters.AddWithValue("@Sn_Olov_Correction", Sn_Olov_Correction);
					//
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 6; // Индекс столбца Date_tech (измените по необходимости)
