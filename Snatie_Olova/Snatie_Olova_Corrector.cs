...
{
	public partial class Snatie_Olova_Corrector : Form
	{
		public Snatie_Olova_Corrector()
		{
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Sn_Olov_Correction, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Snatie_Olova ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Snatie_Olova_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Sn_Olov_Correction, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Snatie_Olova WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
		{
			foreach (DataGridViewRow row in dataGridView.Rows)
			{
				...
				if (!string.IsNullOrEmpty(row.Cells[1].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[10].Value?.ToString()))
				{
					foreach (int columnIndex in columns)
					{
						...
						if (!string.IsNullOrEmpty(row.Cells[7].Value?.ToString()) && string.IsNullOrEmpty(row.Cells[6].Value?.ToString()))
						{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Sn_Olov_Correction, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Snatie_Olova WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				var Correction_Value = selectedRow.Cells["Sn_Olov_Correction"].Value;
				...
				bool isCorrection_MathNotEmpty = Correction_Value != null &&
										  Correction_Value != DBNull.Value &&
										  !(Correction_Value is string strCorrMat && string.IsNullOrWhiteSpace(strCorrMat));
				...
				if (!isCorrection_MathNotEmpty && !isСommentNotEmpty)
				{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out int idClick))
					{
						...
						string query = @"UPDATE Snatie_Olova SET 
                                 [Date_Corr] = @Date_Corr,
                                 [Сompleted] = @Сompleted,
                                 [FIO_Corr] = @FIO_Corr
                                 WHERE ID = @Id";
		...
		private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
		{
			...
			while (currentTableIndex < dataSet.Tables.Count)
			{
				...
				string[] customColumnNames = { "Корр. Плотн. Колич.", "Комментарии" };
