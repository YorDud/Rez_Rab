...
namespace Rez_Lab.Lushenie
{
	public partial class Lushenie_Laborant : Form
	{
		public Lushenie_Laborant()
		{
			InitializeComponent();
			//label3.Text = "Na2S2O8- 30 - 50 г/л\r\nCu 2+ - 5 - 20 г/л\r\nподтрав - 0,3 - 1,2 мкм/мин\r\nОБЪЕМ ВАННЫ: 355 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Lushenie ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Lushen_Na2S2O3"].HeaderText = "Na2S2O8";
				dataGridView1.Columns["Lushen_Cu2"].HeaderText = "Cu 2+";
				dataGridView1.Columns["Lushen_podtrav"].HeaderText = "Подтрав";
				dataGridView1.Columns["Lushen_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Lushen_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Lushen_Na2S2O3"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Lushen_Cu2"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Lushen_podtrav"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Lushenie_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Lushenie WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Lushenie WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Lushenie ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Lushen_Na2S2O3 = dataGridView1.Rows[rowIndex].Cells["Lushen_Na2S2O3"].Value;
				var Lushen_Cu2 = dataGridView1.Rows[rowIndex].Cells["Lushen_Cu2"].Value;
				var Lushen_podtrav = dataGridView1.Rows[rowIndex].Cells["Lushen_podtrav"].Value;
				...
				string updateQuery = @"
            UPDATE Lushenie
   SET [Lushen_Na2S2O3] = @Lushen_Na2S2O3
      ,[Lushen_Cu2] = @Lushen_Cu2
      ,[Lushen_podtrav] = @Lushen_podtrav
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					//
					command.Parameters.AddWithValue("@Lushen_Na2S2O3", Lushen_Na2S2O3);
					command.Parameters.AddWithValue("@Lushen_Cu2", Lushen_Cu2);
					command.Parameters.AddWithValue("@Lushen_podtrav", Lushen_podtrav);
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Lushen_Na2S2O3" && column.Name != "Lushen_Cu2" && column.Name != "Lushen_podtrav")
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Lushen_Na2S2O3 = dataGridView1.Rows[e.RowIndex].Cells["Lushen_Na2S2O3"].Value;
				var Lushen_Cu2 = dataGridView1.Rows[e.RowIndex].Cells["Lushen_Cu2"].Value;
				var Lushen_podtrav = dataGridView1.Rows[e.RowIndex].Cells["Lushen_podtrav"].Value; 
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.Text = "Na2S2O8";

					Label labelValue1 = new Label()
					{
						Text = "C(Na2S2O8) = ((в - a) * 0,1 * 119)/ V",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					{
						Text = "а - количество 0,1Н раств. перм. калия, израсход. на титров., мл\r\nв - количество 0,1Н раств. перм. калия, израсход. на титров. холост. пробы\r\nV - объем раствора, взятый на анализ, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(((v2 - v1) * 0.1 * 119) / v3, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 0.003177 * 1000 / v2, 2); // Пример формулы (сложение)
