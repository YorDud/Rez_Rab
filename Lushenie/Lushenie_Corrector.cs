...
{
	public partial class Lushenie_Corrector : Form
	{
		public Lushenie_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Lushen_Correction_Mat, Lushen_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Lushenie ORDER BY Date_Create DESC", connection);
				...
				////
				//dataGridView1.Columns["Lushen_Na2S2O3"].HeaderText = "Na2S2O3";
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Lushenie_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Lushen_Correction_Mat, Lushen_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Lushenie WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Lushen_Correction_Mat, Lushen_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Lushenie WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				var Correction_Mat_Value = selectedRow.Cells["Lushen_Correction_Mat"].Value;
				var Correction_Score_Value = selectedRow.Cells["Lushen_Correction_Score"].Value;
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"
            SELECT 
                [Lushen_Correction_Mat],
                [Lushen_Correction_Score],
                [Сomment]
            FROM 
                Lushenie
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '')  OR ([Lushen_Correction_Score] IS NOT NULL AND [Lushen_Correction_Score] <> '')
                );
        ";
