...
{
	public partial class Lushenie_Technolog : Form
	{
		public Lushenie_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Lushen_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Lushen_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Lushenie_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Lushen_Correction_Mat" && column.Name != "Lushen_Correction_Score" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Lushenie SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
