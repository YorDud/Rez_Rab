...
{
	public partial class Himich_Podgotov_Technolog : Form
	{
		public Himich_Podgotov_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Him_Podg_1_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Him_Podg_2_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Him_Podg_3_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Himich_Podgotov_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Him_Podg_1_Correction" && column.Name != "Him_Podg_2_Correction" && column.Name != "Him_Podg_3_Correction" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
    ...
    [Him_Podg_3_Correction] = @Him_Podg_3_Correction
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 15; // Индекс столбца Date_tech (измените по необходимости)
