...
{
	public partial class Trav_Med_2_Technolog : Form
	{
		public Trav_Med_2_Technolog()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				dataGridView1.Columns["Trav_Med_2_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Trav_Med_2_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Trav_Med_2_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		//		string query = @"
  //      UPDATE Trav_Med_2 SET
  //          [Date_tech] = @Date_tech,
  //          Trav_Med_2_Correction = @Сorrection,
  //          FIO_tech = @FIO_tech
  //      WHERE ID = @Id";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Trav_Med_2_Correction_Score" && column.Name != "Trav_Med_2_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
        ...
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Trav_Med_2_Correction_Mat", Trav_Med_2_Correction_Mat ?? DBNull.Value);
					command.Parameters.AddWithValue("@Trav_Med_2_Correction_Score", Trav_Med_2_Correction_Score ?? DBNull.Value);
