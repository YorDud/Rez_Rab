...
{
	public partial class Galvan_Zat_Cu_Technolog : Form
	{
		public Galvan_Zat_Cu_Technolog()
		{
			InitializeComponent();
			//label11.Text = "H2SO4 - 150-160 г/л\r\nОБЪЕМ ВАННЫ: 480 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Galvan_Zat_Cu ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Gal_Zat_1_Cu_H2SO4"].HeaderText = "H2SO4";
				dataGridView1.Columns["Gal_Zat_1_Cu_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Gal_Zat_1_Cu_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Gal_Zat_1_Cu_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Gal_Zat_1_Cu_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Galvan_Zat_Cu_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Gal_Zat_1_Cu_Correction_Score" && column.Name != "Gal_Zat_1_Cu_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Zat_1_Cu_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Gal_Zat_1_Cu_Correction_Mat"].Value;
				var Gal_Zat_1_Cu_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Gal_Zat_1_Cu_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Galvan_Zat_Cu SET
            [Date_tech] = @Date_tech,
            [Gal_Zat_1_Cu_Correction_Mat] = @Gal_Zat_1_Cu_Correction_Mat
			,[Gal_Zat_1_Cu_Correction_Score] = @Gal_Zat_1_Cu_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Gal_Zat_1_Cu_Correction_Mat", Gal_Zat_1_Cu_Correction_Mat);
					command.Parameters.AddWithValue("@Gal_Zat_1_Cu_Correction_Score", Gal_Zat_1_Cu_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Galvan_Zat_Cu WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Galvan_Zat_Cu WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Galvan_Zat_Cu SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Galvan_Zat_Cu ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Zat_1_Cu_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Gal_Zat_1_Cu_Correction_Mat"].Value;
				var Gal_Zat_1_Cu_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Gal_Zat_1_Cu_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Galvan_Zat_Cu
   SET [Gal_Zat_1_Cu_Correction_Mat] = @Gal_Zat_1_Cu_Correction_Mat
			,[Gal_Zat_1_Cu_Correction_Score] = @Gal_Zat_1_Cu_Correction_Score
