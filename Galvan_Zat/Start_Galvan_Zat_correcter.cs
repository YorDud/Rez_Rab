...
namespace Rez_Lab
{
	public partial class Start_Galvan_Zat_Cleaner_correcter : Form
	{
		...
		public Start_Galvan_Zat_Cleaner_correcter()
		{
			InitializeComponent();
			//TimerForButton();
			timer = new Timer();
		...
		private void Start_Galvan_Zat_Cleaner_correcter_Load(object sender, EventArgs e)
		{
...
//		}
		...
		{
			this.Hide();
			Galvan_Zat_CuEl12_Corrector tmc = new Galvan_Zat_CuEl12_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Zat_CuEl34_Corrector tmc = new Galvan_Zat_CuEl34_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		private void Load_Galvan_Zat_Cu()
		{
			string query = @"
         SELECT COUNT(*) 
FROM Galvan_Zat_Cu
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Zat_1_Cu_Correction_Mat] IS NOT NULL AND [Gal_Zat_1_Cu_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') and ([Gal_Zat_2_Correction_Mat] <> 'NaCl' or [Gal_Zat_2_Correction_Mat] is null) 
AND (
    ([Gal_Zat_2_Correction_Mat] IS NOT NULL AND [Gal_Zat_2_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') and ([Gal_Zat_3_Correction_Mat] <> 'NaCl' or [Gal_Zat_3_Correction_Mat] is null)
AND (
    ([Gal_Zat_3_Correction_Mat] IS NOT NULL AND [Gal_Zat_3_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Zat_3_Correction_Score] IS NOT NULL AND [Gal_Zat_3_Correction_Score] <> ''));";
		...
		{
		"!!! Декапир Сu !!!",
		"!!! Cu электролит (ванна 1-2) !!!",
		"!!! Cu электролит (ванна 3-4) !!!"
		};
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"
            SELECT 
                [Gal_Zat_1_Cu_Correction_Mat],
                [Gal_Zat_1_Cu_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Zat_Cu
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Gal_Zat_1_Cu_Correction_Score] IS NOT NULL AND [Gal_Zat_1_Cu_Correction_Score] <> '')
                );

            SELECT 
                [Gal_Zat_2_Correction_Mat],
                [Gal_Zat_2_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Zat_CuEl12
            WHERE 
            ...
            SELECT 
                [Gal_Zat_3_Correction_Mat],
                [Gal_Zat_3_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Zat_CuEl34
            WHERE 
