...
namespace Rez_Lab
{
	public partial class Start_Galvan_Line_PAL_Cleaner_correcter : Form
	{
		...
		public Start_Galvan_Line_PAL_Cleaner_correcter()
		{
			InitializeComponent();
			//TimerForButton();
			timer = new Timer();
		...
		private void Start_Galvan_Line_PAL_Cleaner_correcter_Load(object sender, EventArgs e)
		{
...
//		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_KisOch_Corrector tmc = new Galvan_Line_PAL_KisOch_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_Microtrav_Corrector tmc = new Galvan_Line_PAL_Microtrav_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_Cu_Corrector tmc = new Galvan_Line_PAL_Cu_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_Sn_Corrector tmc = new Galvan_Line_PAL_Sn_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_CuEl1920_Corrector tmc = new Galvan_Line_PAL_CuEl1920_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_CuEl2122_Corrector tmc = new Galvan_Line_PAL_CuEl2122_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_SnEl_Corrector tmc = new Galvan_Line_PAL_SnEl_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_CuEl2324_Corrector tmc = new Galvan_Line_PAL_CuEl2324_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		private void Galvan_Line_PAL_KisOch()
		{
			string query = @"
        SELECT COUNT(*) 
...
AND (
    ...
    ([Gal_Line_1_Correction_Score] IS NOT NULL AND [Gal_Line_1_Correction_Score] <> ''));";
		...
		private void Galvan_Line_PAL_Microtrav()
		{
			string query = @"
        SELECT COUNT(*) 
...
AND (
    ...
    ([Gal_Line_2_Correction_Score] IS NOT NULL AND [Gal_Line_2_Correction_Score] <> ''));";
		...
		private void Galvan_Line_PAL_Cu()
		{
			string query = @"
        SELECT COUNT(*) 
...
AND (
    ...
    ([Gal_Line_3_Cu_Correction_Score] IS NOT NULL AND [Gal_Line_3_Cu_Correction_Score] <> ''));";
		...
		private void Galvan_Line_PAL_Sn()
		{
			string query = @"
        SELECT COUNT(*) 
...
AND (
    ...
    ([Gal_Line_3_Sn_Correction_Score] IS NOT NULL AND [Gal_Line_3_Sn_Correction_Score] <> ''));";
		...
		private void Galvan_Line_PAL_CuEl1920()
		{
			string query = @"
       SELECT COUNT(*) 
    ...
    AND (
		...
		private void Galvan_Line_PAL_CuEl2122()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') and ([Gal_Line_6_Correction_Mat] <> 'NaCl' or [Gal_Line_6_Correction_Mat] is null) 
AND (
    ...
    ([Gal_Line_6_Correction_Score] IS NOT NULL AND [Gal_Line_6_Correction_Score] <> ''));";
		...
		private void Galvan_Line_PAL_SnEl()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_7_Correction_Mat] IS NOT NULL AND [Gal_Line_7_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
...
SELECT 
                [Gal_Line_7_Correction_Mat],
                [Gal_Line_7_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_PAL_SnEl
            WHERE 
...
SELECT 
                [Trav_Podv_PAL_Correction_Mat],
                [Trav_Podv_PAL_Correction_Score],
                [Сomment]
            FROM 
                Trav_Podv_PAL
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Trav_Podv_PAL_Correction_Score] IS NOT NULL AND [Trav_Podv_PAL_Correction_Score] <> '')
                );
        ";
		...
		{
			this.Hide();
			Trav_Podv_PAL_Corrector tmc = new Trav_Podv_PAL_Corrector();
			tmc.ShowDialog();
			this.Show();
		}

		private void Trav_Podv_PAL()
		{
			string query = @"
        SELECT COUNT(*) 
FROM 
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				try
				{
					...
					SqlCommand command = new SqlCommand(query, connection);
					int count = (int)command.ExecuteScalar(); // Получение количества выполненных условий
					label10.Text = count.ToString(); // Обновление значения label10
				}
				catch (SqlException ex)
				{
					// Обработка ошибок подключения или выполнения запроса    
