...
{
	public partial class Trav_Podv_PAL_Corrector : Form
	{
		public Trav_Podv_PAL_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Trav_Podv_PAL_Correction_Mat, Trav_Podv_PAL_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Trav_Podv_PAL ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Trav_Podv_PAL_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Trav_Podv_PAL_Correction_Mat, Trav_Podv_PAL_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Trav_Podv_PAL WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Trav_Podv_PAL_Correction_Mat, Trav_Podv_PAL_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Trav_Podv_PAL WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				var Correction_Mat_Value = selectedRow.Cells["Trav_Podv_PAL_Correction_Mat"].Value;
				var Correction_Score_Value = selectedRow.Cells["Trav_Podv_PAL_Correction_Score"].Value;
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"SELECT [Trav_Podv_PAL_Correction_Mat], [Trav_Podv_PAL_Correction_Score], [Date_tech], [Сomment] 
                     FROM Trav_Podv_PAL 
                     WHERE ([Сompleted] IS NULL OR [Сompleted] = '') AND ( 
                         ([Trav_Podv_PAL_Correction_Mat] IS NOT NULL AND [Trav_Podv_PAL_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR  
		...
		private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
		{
			...
			string title = "Травление подвесок";
