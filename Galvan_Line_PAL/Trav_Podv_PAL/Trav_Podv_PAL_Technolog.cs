...
{
	public partial class Trav_Podv_PAL_Technolog : Form
	{
		public Trav_Podv_PAL_Technolog()
		{
			InitializeComponent();
			//label11.Text = "HNO3 - 120-150 г/л\r\nОБЪЕМ ВАННЫ: 1005 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Trav_Podv_PAL ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Trav_Podv_PAL_HNO3"].HeaderText = "HNO3";
				dataGridView1.Columns["Trav_Podv_PAL_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Trav_Podv_PAL_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Trav_Podv_PAL_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Trav_Podv_PAL_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Trav_Podv_PAL_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Trav_Podv_PAL_Correction_Score" && column.Name != "Trav_Podv_PAL_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Trav_Podv_PAL_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Trav_Podv_PAL_Correction_Mat"].Value;
				var Trav_Podv_PAL_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Trav_Podv_PAL_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Trav_Podv_PAL SET
            [Date_tech] = @Date_tech,
            [Trav_Podv_PAL_Correction_Mat] = @Trav_Podv_PAL_Correction_Mat
			,[Trav_Podv_PAL_Correction_Score] = @Trav_Podv_PAL_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Trav_Podv_PAL_Correction_Mat", Trav_Podv_PAL_Correction_Mat);
					command.Parameters.AddWithValue("@Trav_Podv_PAL_Correction_Score", Trav_Podv_PAL_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Trav_Podv_PAL WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Trav_Podv_PAL WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Trav_Podv_PAL SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Trav_Podv_PAL ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Trav_Podv_PAL_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Trav_Podv_PAL_Correction_Mat"].Value;
				var Trav_Podv_PAL_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Trav_Podv_PAL_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Trav_Podv_PAL
   SET [Trav_Podv_PAL_Correction_Mat] = @Trav_Podv_PAL_Correction_Mat
			,[Trav_Podv_PAL_Correction_Score] = @Trav_Podv_PAL_Correction_Score
