...
{
	public partial class Galvan_Line_PAL_SnEl_Laborant : Form
	{
		public Galvan_Line_PAL_SnEl_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Gal_Line_7_H2SO4"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Galvan_Line_PAL_SnEl_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_7_H2SO4 = dataGridView1.Rows[rowIndex].Cells["Gal_Line_7_H2SO4"].Value;
				var Gal_Line_7_SnSO4 = dataGridView1.Rows[rowIndex].Cells["Gal_Line_7_SnSO4"].Value;
   ...
   SET [Gal_Line_7_H2SO4] = @Gal_Line_7_H2SO4, 
[Gal_Line_7_SnSO4] = @Gal_Line_7_SnSO4
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Gal_Line_7_H2SO4" && column.Name != "Gal_Line_7_SnSO4")
				{
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.MinimizeBox = false;
					inputForm.StartPosition = FormStartPosition.CenterParent;
					inputForm.Text = "SnSO4";

					Label labelValue1 = new Label()
					{
						Text = "C(SnSO4) = a * 2,97 * 1,81",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					Label labelValueA = new Label()
					{
						Text = "а - количество 0,01Н раств. йода, израсход. на титров., мл\r\n",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 2.97 * 1.81, 2); // Пример формулы (сложение)
