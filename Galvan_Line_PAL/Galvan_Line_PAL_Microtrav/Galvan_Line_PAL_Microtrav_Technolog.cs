...
{
	public partial class Galvan_Line_PAL_Microtrav_Technolog : Form
	{
		public Galvan_Line_PAL_Microtrav_Technolog()
		{
			InitializeComponent();
			//label11.Text = "Na2S2O8 - 90-120 г/л\r\nCu 2+ - 1-10 г/л\r\nподтрав - 0,6-0,8 мкм/мин\r\nОБЪЕМ ВАННЫ: 1005 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Galvan_Line_PAL_Microtrav ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Gal_Line_2_Na2S2O8"].HeaderText = "Na2S2O8";
				dataGridView1.Columns["Gal_Line_2_Cu2"].HeaderText = "Cu 2+";
				dataGridView1.Columns["Gal_Line_2_podtrav"].HeaderText = "Подтрав";

				dataGridView1.Columns["Gal_Line_2_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Gal_Line_2_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Gal_Line_2_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Gal_Line_2_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Galvan_Line_PAL_Microtrav_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Gal_Line_2_Correction_Score" && column.Name != "Gal_Line_2_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_2_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_2_Correction_Mat"].Value;
				var Gal_Line_2_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_2_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Galvan_Line_PAL_Microtrav SET
            [Date_tech] = @Date_tech,
            [Gal_Line_2_Correction_Mat] = @Gal_Line_2_Correction_Mat
			,[Gal_Line_2_Correction_Score] = @Gal_Line_2_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Gal_Line_2_Correction_Mat", Gal_Line_2_Correction_Mat);
					command.Parameters.AddWithValue("@Gal_Line_2_Correction_Score", Gal_Line_2_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Galvan_Line_PAL_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Galvan_Line_PAL_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Galvan_Line_PAL_Microtrav SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Galvan_Line_PAL_Microtrav ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_2_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Gal_Line_2_Correction_Mat"].Value;
				var Gal_Line_2_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Gal_Line_2_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Galvan_Line_PAL_Microtrav
   SET [Gal_Line_2_Correction_Mat] = @Gal_Line_2_Correction_Mat
			,[Gal_Line_2_Correction_Score] = @Gal_Line_2_Correction_Score
