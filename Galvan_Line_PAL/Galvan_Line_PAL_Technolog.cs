...
{
	public partial class Galvan_Line_PAL_Technolog : Form
	{
		public Galvan_Line_PAL_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				string[] columns = {
	"Gal_Line_1_Correction",
	"Gal_Line_2_Correction",
	"Gal_Line_3_Cu_Correction",
	"Gal_Line_3_Sn_Correction",
	"Gal_Line_4_Correction",
	"Gal_Line_5_Correction",
	"Gal_Line_6_Correction",
	"Gal_Line_7_Correction"
};

				// Установка цвета фона каждому указанному столбцу
				foreach (var columnName in columns)
				{
					dataGridView1.Columns[columnName].DefaultCellStyle.BackColor = Color.PeachPuff;
				}
		...
		private void Galvan_Line_PAL_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			column.Name != "Gal_Line_6_Correction" &&
			column.Name != "Gal_Line_7_Correction" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_7_Correction = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_7_Correction"].Value;
...
[Gal_Line_7_Correction] = @Gal_Line_7_Correction
        ...
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Gal_Line_7_Correction", Gal_Line_7_Correction);
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 30; // Индекс столбца Date_tech (измените по необходимости)
