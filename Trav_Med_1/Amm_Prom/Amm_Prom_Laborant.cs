...
namespace Rez_Lab.Amm_Prom
{
	...
	public partial class Amm_Prom_Laborant : Form
	{

		public Amm_Prom_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Amm_Prom_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
   ...
   SET [Trav_Med_NH4C] = @Trav_Med_NH4C
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Trav_Med_NH4C") // предполагается, что "ID" - это столбец, который можно редактировать
				{
   ...
   SET [Trav_Med_NH4C] = @Trav_Med_NH4C
	...
	WHERE ID = @ID";
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			//			if (double.TryParse(value1.Text, out double v1) && double.TryParse(value2.Text, out double v2))
			//			{
			//				double result = Math.Round(v1 / v2, 2); // Пример формулы (вычитание)
