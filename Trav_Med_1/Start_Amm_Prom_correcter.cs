...
namespace Rez_Lab
{
	public partial class Start_Amm_Prom_correcter : Form
	{
		...
		public Start_Amm_Prom_correcter()
		{
			InitializeComponent();
			//TimerForButton();
			timer = new Timer();
		...
		private void Start_Amm_Prom_correcter_Load(object sender, EventArgs e)
		{
...
//		}
		...
		{
			this.Hide();
			Amm_Prom_Corrector tmc = new Amm_Prom_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
...
";
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"
      SELECT 
          [Trav_Med_Correction_Mat],
          [Trav_Med_Correction_Score],
          [Сomment]
      FROM Trav_Med_1 
      WHERE 
