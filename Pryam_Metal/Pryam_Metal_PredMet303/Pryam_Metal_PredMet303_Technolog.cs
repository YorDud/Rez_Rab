...
{
	public partial class Pryam_Metal_PredMet303_Technolog : Form
	{
		public Pryam_Metal_PredMet303_Technolog()
		{
			InitializeComponent();
			//label11.Text = "pH - < 0,6\r\nCu 2+ < 0,5 \r\nОБЪЕМ ВАННЫ: 185 л"; ;
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Pryam_Metal_PredMet303_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_2_Correction_Score" && column.Name != "Pr_Met_2_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
        UPDATE Pryam_Metal_PredMet303 SET
            [Date_tech] = @Date_tech,
