...
{
	public partial class Pryam_Metal_PredMet_Laborant : Form
	{
		public Pryam_Metal_PredMet_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Pryam_Metal_PredMet_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
...
[Pr_Met_2_Cu2] = @Pr_Met_2_Cu2
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_2_HCl" && column.Name != "Pr_Met_2_pH" && column.Name != "Pr_Met_2_Cu2")
				{
...
[Pr_Met_2_Cu2] = @Pr_Met_2_Cu2
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					{
						Text = "а - количество 0,1Н раствора NaOH, израсходованного на титрование, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 3.65, 2); // Пример формулы (вычитание)
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Date_Lab_Update", Date_Lab_Update);
					command.Parameters.AddWithValue("@FIO_Lab_Update", FIO_Lab_Update);
					command.Parameters.AddWithValue("@Pr_Met_2_HCl", Pr_Met_2_HCl ?? DBNull.Value);
