...
{
	public partial class Pryam_Metal_Corrector : Form
	{
		public Pryam_Metal_Corrector()
		{
			InitializeComponent();
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Pryam_Metal_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
		{
			foreach (DataGridViewRow row in dataGridView.Rows)
			{
				...
				if (!string.IsNullOrEmpty(row.Cells[5].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[9].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[14].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[18].Value?.ToString()))
				{
