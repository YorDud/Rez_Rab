...
{
	public partial class Pryam_Metal_Uskor_Laborant : Form
	{
		public Pryam_Metal_Uskor_Laborant()
		{
			InitializeComponent();
			//label11.Text = "ПМ 305 А - 90 - 100 %\r\nПМ 305 Б - 90 - 100 %\r\nПМ 305 В -  190 - 220 мг/л\r\nОБЪЕМ ВАННЫ: 205 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Pryam_Metal_Uskor ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Pr_Met_4_PM305A"].HeaderText = "ПМ 305 А";
				dataGridView1.Columns["Pr_Met_4_PM305b"].HeaderText = "ПМ 305 Б";
				dataGridView1.Columns["Pr_Met_4_PM305V"].HeaderText = "ПМ 305 В";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Pryam_Metal_Uskor_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Pryam_Metal_Uskor WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Pryam_Metal_Uskor WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Pryam_Metal_Uskor ([Date_Create],[FIO_Lab]) " +
   ...
   SET [Pr_Met_4_PM305A] = @Pr_Met_4_PM305A, 
...
[Pr_Met_4_PM305V] = @Pr_Met_4_PM305V
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_4_PM305A" && column.Name != "Pr_Met_4_PM305b" && column.Name != "Pr_Met_4_PM305V")
				{
...
[Pr_Met_4_PM305V] = @Pr_Met_4_PM305V
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					{
						Text = "а1 - количество 0,1Н соляной кислоты, израсход. на титров. 1 раств, мл\r\nа2 - количество 0,1Н соляной кислоты, израсход. на титров. 2 раств, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round((v1 - v2) * 7.04, 2); // Пример формулы (вычитание)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 5) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					Label labelValueA = new Label()
					{
						Text = "Н - показание портативного колориметра",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (double.TryParse(value1.Text, out double v1))//&& double.TryParse(value2.Text, out double v2))
						{
							double result = Math.Round(v1 * 50, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					Label labelValueA = new Label()
					{
						Text = "а2 - количество 0,1Н соляной кислоты, израсход. на титров. 2 раств, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (double.TryParse(value1.Text, out double v1))//&& double.TryParse(value2.Text, out double v2))
						{
							double result = Math.Round(v1 * 25, 2); // Пример формулы (сложение)
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Date_Lab_Update", Date_Lab_Update);
					command.Parameters.AddWithValue("@FIO_Lab_Update", FIO_Lab_Update);
					command.Parameters.AddWithValue("@Pr_Met_4_PM305A", Pr_Met_4_PM305A ?? DBNull.Value);
					command.Parameters.AddWithValue("@Pr_Met_4_PM305b", Pr_Met_4_PM305b ?? DBNull.Value);
					command.Parameters.AddWithValue("@Pr_Met_4_PM305V", Pr_Met_4_PM305V ?? DBNull.Value);
					command.Parameters.AddWithValue("@ID", id); // ID - это первичный ключ
