...
{
	public partial class Perm_Obra_Sens_Technolog : Form
	{
		public Perm_Obra_Sens_Technolog()
		{
			InitializeComponent();
			//label11.Text = "ПO 401A - 0,68-0,9 норм.\r\nПО 401Б - 190 - 210 г/л\r\nОБЪЕМ ВАННЫ: 370 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Perm_Obra_Sens ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Per_Obr_1_401A"].HeaderText = "ПO 401A";
				dataGridView1.Columns["Per_Obr_1_401b"].HeaderText = "ПO 401Б";
				dataGridView1.Columns["Per_Obr_1_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Per_Obr_1_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Per_Obr_1_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Per_Obr_1_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Perm_Obra_Sens_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Per_Obr_1_Correction_Score" && column.Name != "Per_Obr_1_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_1_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_1_Correction_Mat"].Value;
				var Per_Obr_1_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_1_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Perm_Obra_Sens SET
            [Date_tech] = @Date_tech,
            [Per_Obr_1_Correction_Mat] = @Per_Obr_1_Correction_Mat
			,[Per_Obr_1_Correction_Score] = @Per_Obr_1_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Per_Obr_1_Correction_Mat", Per_Obr_1_Correction_Mat);
					command.Parameters.AddWithValue("@Per_Obr_1_Correction_Score", Per_Obr_1_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Perm_Obra_Sens WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Perm_Obra_Sens WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Perm_Obra_Sens SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Perm_Obra_Sens ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_1_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Per_Obr_1_Correction_Mat"].Value;
				var Per_Obr_1_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Per_Obr_1_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Perm_Obra_Sens
   SET [Per_Obr_1_Correction_Mat] = @Per_Obr_1_Correction_Mat
			,[Per_Obr_1_Correction_Score] = @Per_Obr_1_Correction_Score
