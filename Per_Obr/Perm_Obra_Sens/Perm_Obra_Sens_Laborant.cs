...
{
	public partial class Perm_Obra_Sens_Laborant : Form
	{
		public Perm_Obra_Sens_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Perm_Obra_Sens_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
...
[Per_Obr_1_401b] = @Per_Obr_1_401b
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Per_Obr_1_401A" && column.Name != "Per_Obr_1_401b")
				{
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.Text = "ПО 401А";

					Label labelValue1 = new Label()
					{
						Text = "C(ПО 401А) = a * 0.05",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 0.05, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4)
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.MinimizeBox = false;
					inputForm.StartPosition = FormStartPosition.CenterParent;
					inputForm.Text = "ПО 401Б";

					// Создание и настройка меток для полей ввода
					Label labelValue1 = new Label()
					{
						Text = "Концентр. ПО 401Б, мл/л",
						Top = 5,
						Left = 10,
						Width = 220
					};
					...
					double[] doubleArray = { 220, 200, 180, 160, 140 };

					// Преобразование в object[] и добавление в Items
					value2.Items.AddRange(doubleArray.Cast<object>().ToArray());
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (isV2Selected)
						{
							double v2 = Convert.ToDouble(value2.SelectedItem);
