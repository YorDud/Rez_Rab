...
{
	public partial class Per_Obr_Corrector : Form
	{
		public Per_Obr_Corrector()
		{
			InitializeComponent();
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Per_Obr_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
		{
			foreach (DataGridViewRow row in dataGridView.Rows)
			{
				...
				if (!string.IsNullOrEmpty(row.Cells[5].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[13].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[16].Value?.ToString()) || !string.IsNullOrEmpty(row.Cells[19].Value?.ToString()))
				{
