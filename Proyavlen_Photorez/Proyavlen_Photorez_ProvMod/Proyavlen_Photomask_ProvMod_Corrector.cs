...
{
	public partial class Proyavlen_Photorez_ProvMod_Corrector : Form
	{
		public Proyavlen_Photorez_ProvMod_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Pro_Photorez_1_Correction_Mat, Pro_Photorez_1_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Proyavlen_Photorez_ProvMod ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Proyavlen_Photorez_ProvMod_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Pro_Photorez_1_Correction_Mat, Pro_Photorez_1_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Proyavlen_Photorez_ProvMod WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out int idClick))
					{
						...
						string query = @"UPDATE Proyavlen_Photorez_ProvMod SET 
                                 [Date_Corr] = @Date_Corr,
                                 [Сompleted] = @Сompleted,
                                 [FIO_Corr] = @FIO_Corr
                                 WHERE ID = @Id";
