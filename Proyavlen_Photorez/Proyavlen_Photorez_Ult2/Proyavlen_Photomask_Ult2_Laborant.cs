...
{
	public partial class Proyavlen_Photorez_Ult2_Laborant : Form
	{
		public Proyavlen_Photorez_Ult2_Laborant()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Proyavlen_Photorez_Ult2 ORDER BY Date_Create DESC", connection);
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Proyavlen_Photorez_Ult2_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Proyavlen_Photorez_Ult2 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Proyavlen_Photorez_Ult2 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Proyavlen_Photorez_Ult2 ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Proyavlen_Photorez_Ult2
   SET [Pro_Photorez_3_CO32] = @Pro_Photorez_3_CO32,
...
[Pro_Photorez_3_A_B] = @Pro_Photorez_3_A_B
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pro_Photorez_3_CO32" && column.Name != "Pro_Photorez_3_CO3" && column.Name != "Pro_Photorez_3_pH" && column.Name != "Pro_Photorez_3_A_B")
				{
