...
{
	public partial class Black_Hole_BHC2_Technolog : Form
	{
		public Black_Hole_BHC2_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Black_Hole_6_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Black_Hole_6_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Black_Hole_BHC2_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Black_Hole_6_Correction_Score" && column.Name != "Black_Hole_6_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Black_Hole_6_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Black_Hole_6_Correction_Mat"].Value;
				var Black_Hole_6_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Black_Hole_6_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Black_Hole_BHC2 SET
            [Date_tech] = @Date_tech,
            [Black_Hole_6_soder_tv_chast] = @Black_Hole_6_soder_tv_chast,
[Black_Hole_6_pH] = @Black_Hole_6_pH,
[Black_Hole_6_Correction_Mat] = @Black_Hole_6_Correction_Mat,
[Black_Hole_6_Correction_Score] = @Black_Hole_6_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Black_Hole_6_Correction_Mat", Black_Hole_6_Correction_Mat);
					command.Parameters.AddWithValue("@Black_Hole_6_Correction_Score", Black_Hole_6_Correction_Score);
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Black_Hole_BHC2 SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Black_Hole_6_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Black_Hole_6_Correction_Mat"].Value;
				var Black_Hole_6_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Black_Hole_6_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
