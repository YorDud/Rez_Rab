...
{
	public partial class Black_Hole_BAK1012_Corrector : Form
	{
		public Black_Hole_BAK1012_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Black_Hole_1_Correction_Mat, Black_Hole_1_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Black_Hole_BAK1012 ORDER BY Date_Create DESC", connection);
...
foreach (DataGridViewRow row in dataGridView1.Rows)
{
		...
		private void Black_Hole_BAK1012_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Black_Hole_1_Correction_Mat, Black_Hole_1_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Black_Hole_BAK1012 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Black_Hole_1_Correction_Mat, Black_Hole_1_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Black_Hole_BAK1012 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 7)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out int idClick))
					{
						...
						string query = @"UPDATE Black_Hole_BAK1012 SET 
                                 [Date_Corr] = @Date_Corr,
                                 [Сompleted] = @Сompleted,
                                 [FIO_Corr] = @FIO_Corr
                                 WHERE ID = @Id";
