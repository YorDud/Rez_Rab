...
{
	public partial class Proyavlen_Photomask_Ult2_Technolog : Form
	{
		public Proyavlen_Photomask_Ult2_Technolog()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Proyavlen_Photomask_Ult2 ORDER BY Date_Create DESC", connection);
				...
				dataGridView1.Columns["Pro_Photomask_3_CO3"].HeaderText = "HCO3-";
				dataGridView1.Columns["Pro_Photomask_3_pH"].HeaderText = "pH";
				dataGridView1.Columns["Pro_Photomask_3_A_B"].HeaderText = "A/B";
				dataGridView1.Columns["Pro_Photomask_3_Correction_Mat"].HeaderText = "Корректировка материал";
				dataGridView1.Columns["Pro_Photomask_3_Correction_Score"].HeaderText = "Корректировка количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Pro_Photomask_3_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pro_Photomask_3_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Proyavlen_Photomask_Ult2_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pro_Photomask_3_Correction_Score" && column.Name != "Pro_Photomask_3_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pro_Photomask_3_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Pro_Photomask_3_Correction_Mat"].Value;
				var Pro_Photomask_3_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Pro_Photomask_3_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Proyavlen_Photomask_Ult2 SET
            [Date_tech] = @Date_tech,
            [Pro_Photomask_3_CO32] = @Pro_Photomask_3_CO32,
[Pro_Photomask_3_CO3] = @Pro_Photomask_3_CO3,
[Pro_Photomask_3_pH] = @Pro_Photomask_3_pH,
[Pro_Photomask_3_Correction_Mat] = @Pro_Photomask_3_Correction_Mat,
[Pro_Photomask_3_Correction_Score] = @Pro_Photomask_3_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Pro_Photomask_3_Correction_Mat", Pro_Photomask_3_Correction_Mat);
					command.Parameters.AddWithValue("@Pro_Photomask_3_Correction_Score", Pro_Photomask_3_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Proyavlen_Photomask_Ult2 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Proyavlen_Photomask_Ult2 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Proyavlen_Photomask_Ult2 SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Proyavlen_Photomask_Ult2 ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pro_Photomask_3_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Pro_Photomask_3_Correction_Mat"].Value;
				var Pro_Photomask_3_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Pro_Photomask_3_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Proyavlen_Photomask_Ult2
   SET [Pro_Photomask_3_CO32] = @Pro_Photomask_3_CO32,
[Pro_Photomask_3_CO3] = @Pro_Photomask_3_CO3,
[Pro_Photomask_3_pH] = @Pro_Photomask_3_pH,
[Pro_Photomask_3_Correction_Mat] = @Pro_Photomask_3_Correction_Mat,
[Pro_Photomask_3_Correction_Score] = @Pro_Photomask_3_Correction_Score
