...
{
	public partial class Proyavlen_Photomask_Technolog : Form
	{
		public Proyavlen_Photomask_Technolog()
		{
			InitializeComponent();
		}
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Pro_Photomask_1_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pro_Photomask_2_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pro_Photomask_3_Correction"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Proyavlen_Photomask_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		//		string query = @"
		//      UPDATE Proyavlen_Photomask SET
		//          [Date_tech] = @Date_tech,
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pro_Photomask_1_Correction" && 
					column.Name != "Pro_Photomask_2_Correction" && 
					column.Name != "Pro_Photomask_3_Correction" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
			{
				...
				if (idValue != null)
				{
					...
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string query = @"UPDATE Proyavlen_Photomask SET
                [Date_tech] = @Date_tech,
                FIO_tech = @FIO_tech
            WHERE ID = @Id";
