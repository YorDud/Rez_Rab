...
{
	public partial class Galvan_Line_MG_Microtrav_Laborant : Form
	{
		public Galvan_Line_MG_Microtrav_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Galvan_Line_MG_Microtrav_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
   ...
   SET [Gal_Line_2_NH4_2S2O8] = @Gal_Line_2_NH4_2S2O8, 
...
[Gal_Line_2_podtrav] = @Gal_Line_2_podtrav
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Gal_Line_2_NH4_2S2O8" && column.Name != "Gal_Line_2_Cu2" && column.Name != "Gal_Line_2_podtrav")
				{
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.Text = "(NH4)2S2O8";

					Label labelValue1 = new Label()
					{
						Text = "C((NH4)2S2O8) = ((в - a) * 0,1 * 114,1)/ V",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(((v2 - v1) * 0.1 * 114.1) / v3, 2); // Пример формулы (сложение)
