...
{
	public partial class Galvan_Line_MG_Microtrav_Corrector : Form
	{
		public Galvan_Line_MG_Microtrav_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Gal_Line_2_Correction_Mat, Gal_Line_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Galvan_Line_MG_Microtrav ORDER BY Date_Create DESC", connection);
				...
				////
				//dataGridView1.Columns["Gal_Line_2_NH4_2S2O8"].HeaderText = "AC-22C";
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Galvan_Line_MG_Microtrav_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Gal_Line_2_Correction_Mat, Gal_Line_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Galvan_Line_MG_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Gal_Line_2_Correction_Mat, Gal_Line_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Galvan_Line_MG_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
