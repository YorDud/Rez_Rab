...
{
	public partial class Galvan_Line_MG_Microtrav_Technolog : Form
	{
		public Galvan_Line_MG_Microtrav_Technolog()
		{
			InitializeComponent();
			//label11.Text = "(NH4)2S2O8 - 90-120 г/л\r\nCu 2+ - 1-10 г/л\r\nподтрав - 0,6-0,8 мкм/мин\r\nОБЪЕМ ВАННЫ: 630 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Galvan_Line_MG_Microtrav ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Gal_Line_2_NH4_2S2O8"].HeaderText = "(NH4)2S2O8";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Galvan_Line_MG_Microtrav_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
        UPDATE Galvan_Line_MG_Microtrav SET
            [Date_tech] = @Date_tech,
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Galvan_Line_MG_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Galvan_Line_MG_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Galvan_Line_MG_Microtrav SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Galvan_Line_MG_Microtrav ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Galvan_Line_MG_Microtrav
