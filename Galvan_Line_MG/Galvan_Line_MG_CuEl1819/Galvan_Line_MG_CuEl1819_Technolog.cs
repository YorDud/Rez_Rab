...
{
	public partial class Galvan_Line_MG_CuEl1819_Technolog : Form
	{
		public Galvan_Line_MG_CuEl1819_Technolog()
		{
			InitializeComponent();
			//label11.Text = "H2SO4 - 180-200 г/л           NaCl - 40-60 мг/л\r\nПМ624А - 2-3 мл/л             ПМ624Б - 7-11 мл/л\r\nСuSO4 - 80-100 г/л\r\nОБЪЕМ ВАННЫ: 2300 л ";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Galvan_Line_MG_CuEl1819 ORDER BY Date_Create DESC", connection);
				...
				dataGridView1.Columns["Gal_Line_4_PM624A"].HeaderText = "ПМ 624A (блеск)";
				dataGridView1.Columns["Gal_Line_4_PM624b"].HeaderText = "ПМ 624Б (подавитель)";
				dataGridView1.Columns["Gal_Line_4_CuSO4"].HeaderText = "CuSO4";
				dataGridView1.Columns["Gal_Line_Ah"].HeaderText = "Ah";
				dataGridView1.Columns["Gal_Line_4_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Gal_Line_4_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Gal_Line_4_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Gal_Line_4_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Galvan_Line_MG_CuEl1819_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Gal_Line_4_Correction_Score" && column.Name != "Gal_Line_4_Correction_Mat" && column.Name != "Сomment" && column.Name != "Gal_Line_Ah") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_Ah = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_Ah"].Value;
				var Gal_Line_4_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_4_Correction_Mat"].Value;
				var Gal_Line_4_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Gal_Line_4_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Galvan_Line_MG_CuEl1819 SET
            [Date_tech] = @Date_tech,
            [Gal_Line_Ah] = @Gal_Line_Ah,
            [Gal_Line_4_Correction_Mat] = @Gal_Line_4_Correction_Mat
			,[Gal_Line_4_Correction_Score] = @Gal_Line_4_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Gal_Line_Ah", Gal_Line_Ah);
					command.Parameters.AddWithValue("@Gal_Line_4_Correction_Mat", Gal_Line_4_Correction_Mat);
					command.Parameters.AddWithValue("@Gal_Line_4_Correction_Score", Gal_Line_4_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Galvan_Line_MG_CuEl1819 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Galvan_Line_MG_CuEl1819 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Galvan_Line_MG_CuEl1819 SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Galvan_Line_MG_CuEl1819 ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Gal_Line_Ah = dataGridView1.Rows[rowIndex].Cells["Gal_Line_Ah"].Value;
				var Gal_Line_4_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Gal_Line_4_Correction_Mat"].Value;
				var Gal_Line_4_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Gal_Line_4_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Galvan_Line_MG_CuEl1819
   SET [Gal_Line_4_Correction_Mat] = @Gal_Line_4_Correction_Mat
			,[Gal_Line_Ah] = @Gal_Line_Ah
			,[Gal_Line_4_Correction_Score] = @Gal_Line_4_Correction_Score
